...
### R-11.13 — Familier = entité magique contrôlable par le PJ magicien

Le familier (atout permanent magicien D8) est une entité magique avec :
- `controller: player` (le magicien qui l'a) par défaut, peut basculer `auto` si le magicien fait autre chose.
- Une fiche dédiée et un budget propre selon D8 R-8.13.
- Communication/perception spéciale uniquement si une couche d'atout l'autorise (`Sens du familier`, `Passage au familier`, etc.).
- Initiative distincte.

**Statut** : 🟡 remplacé/complété par R-11.22 et D8 R-8.13
...
### R-11.22 - Familiers magiques (fiche dédiée, clôt Q-D9.47)

**Mise à jour 2026-04-27** : les règles sources du familier ont été retrouvées en D8 R-8.13. Le familier n'est pas un compagnon/monture non magique : il a sa propre fiche, son budget `niveau du magicien × 100`, sa renaissance au passage de niveau et ses atouts traités comme pseudo-raciaux.

Le texte legacy le range parfois avec les "personnages secondaires". Pour la spec, c'est un raccourci historique : les compagnons/montures non magiques restent des personnages/créatures autonomes pouvant évoluer par XP partagé ; les familiers utilisent R-8.13.
...
familiar:
  id: <slug>
  master: <character_id>                  # magicien propriétaire
  item_type: familiar
  bond_type: magical_familiar

  form:
    description: <text>                   # forme libre choisie par le magicien
    volume_cap_vitality_equivalent: <int> # niveau_magicien * 5

  sheet:
    build_points: <int>                   # niveau_magicien * 100
    vitality: <int>                       # min 1, max niveau_magicien * 5 sauf atout
    speed_factor: 8
    will_factor: 8
    aptitudes: {}
    skills: {}
    knowledge: {}                         # jamais au-dessus des connaissances du magicien
    familiar_atouts: []                   # atouts traités comme raciaux
    familiar_handicaps: []
  ...
  is_additional: bool
  inherited_atouts: [<atout_id>]          # atouts transmis/remaniables au passage de niveau
  controller: player | auto | llm
  next_action_at: <int DT>
  current_state: active | dismissed | possessed | dead_until_rebirth

  metadata:
    source_rules_loaded: true             # D8 R-8.13
    atout_value_table: data/catalogs/atouts-values.csv # coût valeur/10
    version: <int>
```

#### B. Mapping atouts → couches

| Atout (lexique) | Couche / effet | Note |
|---|---|---|
| **Familier** | `base` | Donne accès au familier. |
| **Sens du familier** | `sensory_link` | Lien sensoriel sur un sens choisi. |
| **Main du magicien** | `remote_spell_casting` | Le magicien lance un sort depuis la position du familier. |
| **Passage au familier** | `possession_transfer` | Le magicien entre intégralement dans son familier. |
| **Rappel du familier** | `summon_dismiss` | Le familier peut être rappelé en soi ; entrée/sortie prend un facteur de vitesse. |
| **Familier supplémentaire** | `multiplicity` | Ajoute un familier ; répétabilité exacte à préciser si besoin. |
| **Familier vigoureux** | `vitality_bonus` | Augmente la vitalité du familier selon le niveau du propriétaire. |
| **Hybridation du familier** | `body_hybridation` | Substitution d'une zone du corps par celle du familier rappelé. |
| **Ventre du magicien** | `nourishment_link` | Le familier peut nourrir le magicien en se nourrissant. |
| **Vol de familier** | `familiar_theft` | Détourne le lien d'un familier adverse tant que l'atout est actif. |

#### C. Templates de formes

Les formes types sont des **préréglages d'apparence et de fiche**, pas des héritages automatiques des règles d'animaux R-11.21. Un chat familier peut ressembler à un chat, mais il reste créé avec le budget R-8.13.
...
familiar_template:
  ...
  name: "Chat familier"
  suggested_form: cat
  suggested_traits: [agile, stealthy, climber]
```

Formes courantes : chat, corbeau, hibou, serpent, crapaud, faucon, rat, sphère magique, livre ailé, oeil flottant. Les formes non animales utilisent le volume équivalent plutôt qu'une taille animale stricte.

#### D. Initiative et action

- **Initiative distincte** : le familier a son propre `next_action_at`, calculé avec son `speed_factor` propre.
- Si `controller: player`, le PJ magicien choisit l'action du familier.
- Si `controller: auto`, IA tactique simple.
- Si `controller: llm`, LLM avec personnalité du familier injectée.
- Les couches comme `sensory_link`, `remote_spell_casting` ou `possession_transfer` modifient la source de perception, de lancement ou de contrôle, sans transformer le familier en PJ ordinaire.

#### E. Mort, niveau et renaissance

Règle source :
- si le familier meurt, les points investis dans sa fiche courante sont perdus jusqu'à renaissance ;
- un familier mort peut être récupéré au passage de niveau du magicien ;
- au passage de niveau, le budget devient `nouveau niveau × 100` ;
- le budget est total, pas ajouté intégralement à chaque niveau ;
- les points déjà engagés peuvent être conservés, retirés, transformés ou réattribués ;
- le magicien peut conserver ou changer la forme.

Les drains d'énergie ou pertes sensorielles précédemment proposés ne sont **pas canoniques** : ils peuvent rester des variantes de campagne, mais ne doivent pas être appliqués par défaut.

#### F. Points à valider

- Répétabilité exacte de `Familier supplémentaire` et `Familier vigoureux`, si le moteur doit l'automatiser strictement.

#### G. Application par mode arbitre

- **MJ humain** : peut autoriser des formes et combinaisons d'atouts exotiques.
- **MJ LLM** : applique R-8.13, signale les atouts incohérents avec la forme, propose des interprétations.
- **MJ auto** : strict, applique la table générale des valeurs d'atouts (`valeur / 10`).
...
**Contexte** : R-11.2 acte le flag PJ vs PNJ mutable. R-13.10 précise maintenant le lien long terme : partage XP sans limite fixe autre que l'XP libre du propriétaire, progression/mort comme personnage indépendant, partage de loot selon les règles de personnage et les accords sociaux. Distinct des familiers (R-11.22) — un compagnon est plus autonome, peut être PNJ humain/elfe/etc.
...
Choix D — hybride règle vivante, mis à jour par D8 R-8.13 : fiche dédiée du familier + budget total `niveau × 100` + atouts comme pseudo-raciaux + coût `valeur / 10` via table générale des atouts + couches explicites débloquées par atouts. Voir R-11.22 pour la mécanique. Clôture du backlog Q-D9.47.
