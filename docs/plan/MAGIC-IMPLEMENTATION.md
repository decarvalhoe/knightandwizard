...
- **E2 — Audit/structuration des 324 sorts** (catalogs + gouvernance, méthode Q-D8.2) : effet structuré + `direct_magic` + `damage_type` + type élémentaire + catégorie de cumul (Q-D8.9), **depuis la prose du lexique (E0)** et non la ligne terse, un passage par sort. _Dépend de : E0 ✅ + E1 ✅._
  ...
  - **E2b+ — Authoring gouverné** : `EffectModel` par sort, en **overlay git-native** (`data/spell-effects/<école>.yaml`) mergé par `link-spell-effects` (valide via `parseEffectModel`). Cycle par école : authoring `fidelity: pending` + `requires_mj_validation` → **validation MJ** (règles vivantes) → `fidelity: covered` (canon). **État : 204/324 `effect_model` structurés (42 `covered`, 162 `pending`)** sur les **11 écoles** (#213→#231) :
    - **Validés `covered`** (42) : élémentaire 16 (dégâts par réussite) + magie-blanche 26 (soins, buffs d'aptitude, bénédiction, facteurs).
