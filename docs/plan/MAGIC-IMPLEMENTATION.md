...
- **E2 — Audit/structuration des 324 sorts** (catalogs + gouvernance, méthode Q-D8.2) : effet structuré + `direct_magic` + `damage_type` + type élémentaire + catégorie de cumul (Q-D8.9), **depuis la prose du lexique (E0)** et non la ligne terse, un passage par sort. _Dépend de : E0 ✅ + E1 ✅._
  ...
  - **E2b+ — Authoring gouverné** (en cours) : `EffectModel` par sort, en **overlay git-native** (`data/spell-effects/<école>.yaml`) mergé par `link-spell-effects` (valide via `parseEffectModel`). Cycle par école : authoring `fidelity: pending` + `requires_mj_validation` → **validation MJ** (règles vivantes) → `fidelity: covered` (canon). **État : 69 `effect_model` (42 `covered`, 27 `pending`)** sur 5 écoles :
    - **élémentaire** ✅ validé (16, dégâts par réussite, scope élément ; #213/#217).
    - **magie-blanche** : 26 ✅ validés (soins, buffs d'aptitude, bénédiction, facteurs ; #217/#219) + 6 `bouclier*` `pending` (target `protection`, #222).
    - **magie-noire** : 13 `pending` (debuffs d'aptitude, facteurs, malédiction, douleur ; #221).
    - **enchantement** : 1 `pending` (invisibilité) ; **magie-naturelle** : 3 `pending` (traces, obstacles) — buffs de difficulté (#223).
    - Extensions de schéma faites en chemin : variable `successes`, durées narratives (E1a), target `protection` (E2f).
    - _Reste — arbitrage (gaps de schéma identifiés) : **contrôle/charme/peur** (besoin condition de jet opposé + target `status`), **invocations/appels** (création d'entité), **drains** (bi-face cible+lanceur), **divination/illusion/terrain** (narratif). À structurer `pending` au fil de l'eau (arbitrage humain via règles vivantes plus tard)._
