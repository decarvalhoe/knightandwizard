...
- **2026-05-22** — **`packages/ui` créé** (7ᵉ package) : composants réutilisables **branchés tokens, neutres au skin** — `Button` (primary/secondary), `Card`, `Badge` (5 tons feedback), `Label` (eyebrow), `Die` (signature dé/destin success/critical/one) + `styles.css` (états `:active`/`:focus-visible`, `var()` only). typecheck/lint/format verts (turbo compile via `tsc -b`). NB : polices des skins chargées dans `apps/game` (layout, Google Fonts) — c'était la cause du « pas chargé ». **Reste** : brancher `packages/ui` dans `apps/game` (`transpilePackages` + import `@knightandwizard/ui/styles.css`) et l'utiliser dans les surfaces (le proto deviendra une vraie page composée).
- **2026-05-23** — **`packages/ui` étoffé + 1ʳᵉ vraie surface composée**. Deux composants ajoutés (7 → 9) : **`Seal`** (sceau de cire — pastille circulaire `accent-2`, capitale, `rotate(-4deg)`, pour cachets/niveaux) et **`StatBlock`** (grille libellé/valeur `auto-fill minmax(120px)` — aptitudes, attributs de sort…). Index + `styles.css` (`.kw-seal`, `.kw-statblock__*`, var()-only) mis à jour. **Surface : `apps/game/src/app/design-proto/fiche/page.tsx`** — *Fiche de personnage* en **skin `armorial`** (« livret du héros · enrôlement scellé »), composée **100 % avec `@knightandwizard/ui`** : `Label` + titre `Aveline de Fauche-le-Vent`, `Badge` (Guerrière/Humaine·Carénienne), `Seal` « Niv. 3 » (émail azur), `Card` + `StatBlock` des **9 aptitudes**, `Card` test « tir à l'arc » (`Die` 10↪9 réussites / 7 critique / 1, « 2 réussites » + dicton cynique), `Button` « Consigner au registre ». **Validé live Jour + Veillée** (screenshots) : la surface bascule sur la rampe nuit, le Seal/Badge passent au bleu-info nuit, les dés « réussite » s'inversent, le bouton prend l'accent rouge nuit. **Gates verts** (format/lint/typecheck — `tsc -b` recompile le dist `packages/ui` consommé par `apps/game`). Reste (optionnel) : brancher une surface sur des **données réelles** (rules-core/catalogs) ; PR `feat/frontend-foundations` → `dev`.
